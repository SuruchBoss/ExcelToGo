...
### 🧪 สิ่งที่เทสต์ 1058 เคสจับไม่ได้
...
**แล้วมันก็เกิดอีก คนละที่** ตอนที่ทุกด่านเขียวหมด — 1058 เทสต์ผ่าน, `axe` ไม่มี violation ทั้งสองหน้า
...
| `npm test` | รัน unit test 1058 เคสด้วย Vitest |
...
**ยังไม่รองรับ:** sync อัตโนมัติ (กดบันทึกเอง) · ประวัติเวอร์ชัน · แชร์ให้คนอื่น

### 👥 แก้พร้อมกันหลายคน

บันทึกงานขึ้น Supabase ของคุณแล้ว กด **"เข้าห้องแก้ร่วมกัน"** ในแถบคลาวด์ ทุกคนที่เปิดงานเดียวกันจาก
โปรเจกต์เดียวกันจะเห็นการพิมพ์ของกันและกันทันที พร้อมจุดสีบอกว่าใครอยู่ช่องไหน
...
ข้อความทุกชิ้นที่มาจากช่องสัญญาณ**ถูกตรวจก่อนใช้** ไม่ใช่เชื่อ — ใครก็ตามที่ถือ anon key ก็เข้าห้องได้
`row` ที่เป็น `-1` จะไปถึง array index และ `raw` ที่ไม่ใช่สตริงจะไปถึงเอนจินคำนวณสูตร
...
| `vitest` | unit test เอนจินคำนวณสูตร, ตรรกะเรียงข้อมูล, แปลง JSON เป็นตาราง, การแบ่งหน้า/rate limit, แม่แบบ/รูปแบบจากไฟล์ และบล็อกข้อมูลสด (1058 เคส) |
...
```bash
npm test      # 1058 เคส ใน 65 ไฟล์ ด้วย Vitest
```
...
**แต่ 1058 เคสนั้นไม่มีสักเคสที่เปิดแอป** และบั๊กที่โปรเจกต์นี้เจอด้วยการนั่งกดเอง อยู่ที่รอยต่อระหว่างชิ้นส่วนที่ผ่านเทสต์
...
> **1058 เคสผ่านหมด แต่ 43% ของคำตอบจากผู้ช่วย AI ใช้ไม่ได้** — เพราะเทสต์ส่วนนั้น mock ตัวโมเดลไว้
> มันจึงตอบตามที่คนเขียนเทสต์คิดว่ามันจะตอบ จำนวนเทสต์บอกได้แค่ว่าเราถามอะไรไปบ้าง ไม่ได้บอกว่าเราถาม
> ครบหรือยัง เรื่องนี้เจอได้ก็ต่อเมื่อเอา API key จริงมายิง — ดู [สิ่งที่เทสต์ 1058 เคสจับไม่ได้](#-สิ่งที่เทสต์-1058-เคสจับไม่ได้)
...
| `cloud/realtimeChannel.test.ts` | 5 | อ่าน presence เป็นรายชื่อคน — จุดที่ `id` กับ `from` ไม่ตรงกันแล้วกลายเป็น "ไม่มีใครอยู่เลย" อย่างเงียบ ๆ |
...
- [x] **บันทึกลงคลาวด์ / sync ข้ามเครื่อง** — ทำแล้วแบบ **"เอา backend ของคุณเองมาต่อ"**
      ...
      ไม่ใช่บริการที่โฮสต์ให้ ยังเหลือ: sync อัตโนมัติ, ประวัติเวอร์ชัน และการแชร์
