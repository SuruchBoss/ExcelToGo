...
| `npm run check:e2e` | ขับแอปจริงในเบราว์เซอร์ 8 flow: พิมพ์สูตร, ส่งออก-นำเข้า `.xlsx`, คีย์บอร์ดล้วน, undo, เสียงประกาศ, ผู้ช่วย AI, CSP (ต้อง build ก่อน) |
...
| `npm run verify` | รันรวดเดียวก่อน push: lint → check:readme → test → check:mutants → build → check:a11y → check:e2e |
...
```bash
npm run check:e2e   # 8 flow ในเบราว์เซอร์จริง (ต้อง build ก่อน)
```
