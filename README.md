...
(Thai/English), 1067 automated tests. บันทึกบนคลาวด์และการแก้พร้อมกันหลายคนมีให้เลือกใช้ได้ โดยต่อ backend ของคุณเอง
...
### 🧪 สิ่งที่เทสต์ 1067 เคสจับไม่ได้
...
**แล้วมันก็เกิดอีก คนละที่** ตอนที่ทุกด่านเขียวหมด — 1067 เทสต์ผ่าน, `axe` ไม่มี violation ทั้งสองหน้า
...
| `npm test` | รัน unit test 1067 เคสด้วย Vitest |
...
> **ยังไม่รองรับ:** ชีตที่มี array formula จะคำนวณใหม่ทั้งแผ่นทุกครั้งแทนที่จะคำนวณเฉพาะส่วนที่เปลี่ยน
> (`computeSheet` ปฏิเสธเส้นทาง incremental ไปเลยเมื่อเจอ spill) — เพราะ array เขียนลงเซลล์ที่อยู่*นอก*
> ชุดที่ถูกมาร์กว่าต้องคำนวณใหม่ และการลบ array ต้องล้างเซลล์พวกนั้นด้วย การติดตามให้ถูกต้องต้องเอา
> spill เข้าไปอยู่ใน dependency graph ซึ่งใหญ่กว่าที่ฟีเจอร์นี้ควรจ่ายตอนนี้ — และ array ที่ลบไปครึ่งเดียว
> ค้างบนจอ เป็นบั๊กที่แย่กว่าการคำนวณช้าลง · ส่วนไฟล์ `.xlsx` ที่ส่งออกเขียนสูตรไว้ที่เซลล์ต้นทาง
> Excel รุ่นที่รู้จัก dynamic array จะล้นให้เอง รุ่นเก่ากว่านั้นจะเห็นแค่ค่าเดียว
...
| `vitest` | unit test เอนจินคำนวณสูตร, ตรรกะเรียงข้อมูล, แปลง JSON เป็นตาราง, การแบ่งหน้า/rate limit, แม่แบบ/รูปแบบจากไฟล์ และบล็อกข้อมูลสด (1067 เคส) |
...
```bash
npm test      # 1067 เคส ใน 65 ไฟล์ ด้วย Vitest
```
...
**แต่ 1067 เคสนั้นไม่มีสักเคสที่เปิดแอป** และบั๊กที่โปรเจกต์นี้เจอด้วยการนั่งกดเอง อยู่ที่รอยต่อระหว่างชิ้นส่วนที่ผ่านเทสต์
...
> **1067 เคสผ่านหมด แต่ 43% ของคำตอบจากผู้ช่วย AI ใช้ไม่ได้** — เพราะเทสต์ส่วนนั้น mock ตัวโมเดลไว้
> มันจึงตอบตามที่คนเขียนเทสต์คิดว่ามันจะตอบ จำนวนเทสต์บอกได้แค่ว่าเราถามอะไรไปบ้าง ไม่ได้บอกว่าเราถาม
> ครบหรือยัง เรื่องนี้เจอได้ก็ต่อเมื่อเอา API key จริงมายิง — ดู [สิ่งที่เทสต์ 1067 เคสจับไม่ได้](#-สิ่งที่เทสต์-1067-เคสจับไม่ได้)
...
- [x] **สูตรที่ตอบมาเป็นทั้งตาราง (array formula)** — ทำแล้ว (ดู[สูตรที่ตอบมาเป็นทั้งตาราง (array formula)](#-สูตรที่ตอบมาเป็นทั้งตาราง-array-formula)):
      ...
      และตัวดำเนินการที่ทำงานทั้งช่วง (`A1:A9>50` คือเก้าคำตอบ) ยังเหลือ: ชีตที่มี array คำนวณใหม่ทั้งแผ่น
      แทนที่จะคำนวณเฉพาะส่วนที่เปลี่ยน และยังไม่มี `XMATCH`/`LET`/`LAMBDA`
