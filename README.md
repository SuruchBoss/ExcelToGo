...
(Thai/English), 319 automated tests.
...
| `npm test` | รัน unit test 319 เคสด้วย Vitest |
...
เลือกช่วง กดปุ่ม **กราฟ** ในแถบรูปแบบ แล้วเลือก **แท่ง / เส้น / วงกลม** — สลับชนิดทีหลังได้ตลอด
...
**ยังไม่รองรับ:** ลากย้าย/ปรับขนาดกราฟบนตารางแบบ Excel (กราฟอยู่ในพาเนลด้านข้าง) · ส่งกราฟออกไปกับไฟล์
`.xlsx` หรือ PDF · กราฟวงกลมใช้ชุดข้อมูลแรกชุดเดียว
...
| `vitest` | unit test เอนจินคำนวณสูตร, ตรรกะเรียงข้อมูล, แปลง JSON เป็นตาราง, การแบ่งหน้า/rate limit, แม่แบบ/รูปแบบจากไฟล์ และบล็อกข้อมูลสด (319 เคส) |
...
src/
  ...
  lib/                       # ตรรกะหลักของโดเมน ไม่ผูกกับ React/UI แก้ไข/ทดสอบแยกจาก UI ได้อิสระ
    ...
    charts.ts                # กราฟ: อ่านช่วงเป็นชุดข้อมูล+ป้ายกำกับ, หาแกน, เลื่อนตามการแก้ตาราง (มี test)
...
```bash
npm test      # 319 เคส ใน 18 ไฟล์ ด้วย Vitest
```
...
| `charts.test.ts` | 20 | อ่านช่วงเป็นชุดข้อมูล+ป้ายกำกับ (รวมกรณีคอลัมน์ซ้ายเป็นข้อความ), ช่องว่างเมื่อไม่ใช่ตัวเลข, แกนที่รวมศูนย์, การเลื่อนตามแทรก/ลบ |
