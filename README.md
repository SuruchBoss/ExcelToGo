...
(Thai/English), 801 automated tests. บันทึกบนคลาวด์มีให้เลือกใช้ได้ โดยต่อ backend ของคุณเอง
...
**แล้วมันก็เกิดอีก คนละที่** ตอนที่ทุกด่านเขียวหมด — 801 เทสต์ผ่าน, `axe` ไม่มี violation ทั้งสองหน้า
...
| `npm test` | รัน unit test 801 เคสด้วย Vitest |
...
| `vitest` | unit test เอนจินคำนวณสูตร, ตรรกะเรียงข้อมูล, แปลง JSON เป็นตาราง, การแบ่งหน้า/rate limit, แม่แบบ/รูปแบบจากไฟล์ และบล็อกข้อมูลสด (801 เคส) |
...
```bash
npm test      # 801 เคส ใน 48 ไฟล์ ด้วย Vitest
```
...
> **801 เคสผ่านหมด แต่ 43% ของคำตอบจากผู้ช่วย AI ใช้ไม่ได้** — เพราะเทสต์ส่วนนั้น mock ตัวโมเดลไว้
> มันจึงตอบตามที่คนเขียนเทสต์คิดว่ามันจะตอบ จำนวนเทสต์บอกได้แค่ว่าเราถามอะไรไปบ้าง ไม่ได้บอกว่าเราถาม
> ครบหรือยัง เรื่องนี้เจอได้ก็ต่อเมื่อเอา API key จริงมายิง — ดู [สิ่งที่เทสต์ 801 เคสจับไม่ได้](#-สิ่งที่เทสต์-801-เคสจับไม่ได้)
...
- [x] **ตารางพูดภาษา ARIA grid แล้ว** — ทำแล้ว: `role="grid"`, จำนวนแถว/คอลัมน์และ index ต่อเซลล์
      ...
      ยังเป็นตารางที่ขับไม่ได้ ยังเหลือ: `aria-live` สรุปสิ่งที่เกิดขึ้นห่างจากเคอร์เซอร์ (เรียงข้อมูล นำเข้า วาง)
      และทั้งหมดนี้ยังไม่ได้ทดสอบกับ screen reader จริง
