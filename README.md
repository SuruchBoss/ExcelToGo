...
(Thai/English), 788 automated tests. บันทึกบนคลาวด์มีให้เลือกใช้ได้ โดยต่อ backend ของคุณเอง
...
**แล้วมันก็เกิดอีก คนละที่** ตอนที่ทุกด่านเขียวหมด — 788 เทสต์ผ่าน, `axe` ไม่มี violation ทั้งสองหน้า
...
| `npm test` | รัน unit test 788 เคสด้วย Vitest |
...
| `vitest` | unit test เอนจินคำนวณสูตร, ตรรกะเรียงข้อมูล, แปลง JSON เป็นตาราง, การแบ่งหน้า/rate limit, แม่แบบ/รูปแบบจากไฟล์ และบล็อกข้อมูลสด (788 เคส) |
...
```bash
npm test      # 788 เคส ใน 47 ไฟล์ ด้วย Vitest
```
...
> **788 เคสผ่านหมด แต่ 43% ของคำตอบจากผู้ช่วย AI ใช้ไม่ได้** — เพราะเทสต์ส่วนนั้น mock ตัวโมเดลไว้
> มันจึงตอบตามที่คนเขียนเทสต์คิดว่ามันจะตอบ จำนวนเทสต์บอกได้แค่ว่าเราถามอะไรไปบ้าง ไม่ได้บอกว่าเราถาม
> ครบหรือยัง เรื่องนี้เจอได้ก็ต่อเมื่อเอา API key จริงมายิง — ดู [สิ่งที่เทสต์ 788 เคสจับไม่ได้](#-สิ่งที่เทสต์-788-เคสจับไม่ได้)
...
- [ ] **หน้ารวมคีย์ลัดในแอป** — ปุ่มแบบ Excel (`Ctrl+↓`, `Ctrl+A` สองจังหวะ, `Home`/`End`, ปุ่ม `Page`)
      ทำงานได้ทั้งหมด แต่ไม่มีอะไรบนจอบอกว่ามันมีอยู่ ลองไล่หาแล้วไม่มีปุ่มช่วยเหลือ ไม่มีรายการคีย์ลัด
      และไม่มีคำว่า `Ctrl+` อยู่ที่ไหนเลยนอกจาก tooltip ของ undo/redo
