...
(Thai/English), 817 automated tests. บันทึกบนคลาวด์มีให้เลือกใช้ได้ โดยต่อ backend ของคุณเอง
...
**แล้วมันก็เกิดอีก คนละที่** ตอนที่ทุกด่านเขียวหมด — 817 เทสต์ผ่าน, `axe` ไม่มี violation ทั้งสองหน้า
...
| `npm test` | รัน unit test 817 เคสด้วย Vitest |
...
ข้อมูลสดจงใจไม่อ่าน เพราะมันดึงข้อมูลใหม่ทุกไม่กี่วินาที region ที่พูดทุกห้าวินาทีไม่ใช่ฟีเจอร์ช่วยการเข้าถึง
แต่เป็นความผิดพลาด กราฟกับ pivot ก็ยังไม่ครอบคลุม และ**ยังไม่มี screen reader ตัวจริงได้ฟังสักที** —
สิ่งที่ยืนยันได้คือข้อความที่ถูกไปถึง region ที่ถูก ในจังหวะที่ถูก
...
| `vitest` | unit test เอนจินคำนวณสูตร, ตรรกะเรียงข้อมูล, แปลง JSON เป็นตาราง, การแบ่งหน้า/rate limit, แม่แบบ/รูปแบบจากไฟล์ และบล็อกข้อมูลสด (817 เคส) |
...
```bash
npm test      # 817 เคส ใน 50 ไฟล์ ด้วย Vitest
```
...
> **817 เคสผ่านหมด แต่ 43% ของคำตอบจากผู้ช่วย AI ใช้ไม่ได้** — เพราะเทสต์ส่วนนั้น mock ตัวโมเดลไว้
> มันจึงตอบตามที่คนเขียนเทสต์คิดว่ามันจะตอบ จำนวนเทสต์บอกได้แค่ว่าเราถามอะไรไปบ้าง ไม่ได้บอกว่าเราถาม
> ครบหรือยัง เรื่องนี้เจอได้ก็ต่อเมื่อเอา API key จริงมายิง — ดู [สิ่งที่เทสต์ 817 เคสจับไม่ได้](#-สิ่งที่เทสต์-817-เคสจับไม่ได้)
...
- [x] **อ่านออกเสียงสิ่งที่เกิดขึ้นห่างจากเคอร์เซอร์** — ทำแล้ว: live region แบบ polite บอกว่าการเรียง กรอง วาง
      ...
      ส่วนการเลื่อนเคอร์เซอร์เงียบไว้ — โฟกัสอ่านชื่อเซลล์ให้อยู่แล้ว พูดซ้ำแย่กว่าไม่พูด ยังเหลือ: ข้อมูลสด
      รีเฟรชตัวเองทุกไม่กี่วินาที จึงจงใจไม่อ่าน กราฟกับ pivot ยังไม่ครอบคลุม และยังไม่มี screen reader
      ตัวจริงมาฟัง
