...
(Thai/English), 1122 automated tests. บันทึกบนคลาวด์และการแก้พร้อมกันหลายคนมีให้เลือกใช้ได้ โดยต่อ backend ของคุณเอง
...
### 🧪 สิ่งที่เทสต์ 1122 เคสจับไม่ได้
...
**แล้วมันก็เกิดอีก คนละที่** ตอนที่ทุกด่านเขียวหมด — 1122 เทสต์ผ่าน, `axe` ไม่มี violation ทั้งสองหน้า
...
| `npm test` | รัน unit test 1122 เคสด้วย Vitest |
...
| `vitest` | unit test เอนจินคำนวณสูตร, ตรรกะเรียงข้อมูล, แปลง JSON เป็นตาราง, การแบ่งหน้า/rate limit, แม่แบบ/รูปแบบจากไฟล์ และบล็อกข้อมูลสด (1122 เคส) |
...
```bash
npm test      # 1122 เคส ใน 69 ไฟล์ ด้วย Vitest
```
...
**แต่ 1122 เคสนั้นไม่มีสักเคสที่เปิดแอป** และบั๊กที่โปรเจกต์นี้เจอด้วยการนั่งกดเอง อยู่ที่รอยต่อระหว่างชิ้นส่วนที่ผ่านเทสต์
...
> **1122 เคสผ่านหมด แต่ 43% ของคำตอบจากผู้ช่วย AI ใช้ไม่ได้** — เพราะเทสต์ส่วนนั้น mock ตัวโมเดลไว้
> มันจึงตอบตามที่คนเขียนเทสต์คิดว่ามันจะตอบ จำนวนเทสต์บอกได้แค่ว่าเราถามอะไรไปบ้าง ไม่ได้บอกว่าเราถาม
> ครบหรือยัง เรื่องนี้เจอได้ก็ต่อเมื่อเอา API key จริงมายิง — ดู [สิ่งที่เทสต์ 1122 เคสจับไม่ได้](#-สิ่งที่เทสต์-1122-เคสจับไม่ได้)
...
- [x] **คีย์บอร์ดให้ครบแบบ Excel** — ทำแล้ว (ดูหัวข้อ [คีย์บอร์ดแบบเดียวกับ Excel](#️-คีย์บอร์ดแบบเดียวกับ-excel)):
      ...
      ไม่งั้นจะได้คอลัมน์ที่เป็นเลขผิดเหมือนกันหมด) ยังเหลือ: ยังไม่มี freeze pane นอกเหนือจากหัวตารางที่ติดอยู่แล้ว
