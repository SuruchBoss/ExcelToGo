...
**ยังไม่รองรับ:** ส่งกราฟออกไปกับไฟล์ `.xlsx` หรือ PDF · กราฟวงกลมใช้ชุดข้อมูลแรกชุดเดียว ·
ตำแหน่งกราฟเป็นพิกเซล ไม่ได้ยึดกับเซลล์ จึงอยู่ที่เดิมเมื่อแทรกคอลัมน์ทางซ้าย (ช่วงที่อ่านเลื่อนตามให้)
...
**ยังไม่รองรับ:** ลากเลือกหลายเซลล์ด้วยนิ้ว (แตะทีละช่องได้) · เมนูคลิกขวาที่หัวแถว/คอลัมน์ต้องกดค้าง
ซึ่งบางเบราว์เซอร์บนมือถือจะเด้งเมนูของตัวเองมาแทน
...
src/
  ...
  lib/                       # ตรรกะหลักของโดเมน ไม่ผูกกับ React/UI แก้ไข/ทดสอบแยกจาก UI ได้อิสระ
    ...
    charts.ts                # กราฟ: อ่านช่วงเป็นชุดข้อมูล+ป้ายกำกับ, หาแกน, กรอบวาง/ย้าย/ย่อขยายบนตาราง,
                              # คำอธิบายสีตามชนิดกราฟ, เลื่อนตามการแก้ตาราง (มี test)
...
| `charts.test.ts` | 33 | อ่านช่วงเป็นชุดข้อมูล+ป้ายกำกับ (รวมกรณีคอลัมน์ซ้ายเป็นข้อความ), ช่องว่างเมื่อไม่ใช่ตัวเลข, แกนที่รวมศูนย์, การย้าย/ย่อขยาย/หนีบกรอบไว้ในชีต, คำอธิบายสีตามชนิดกราฟ, การเลื่อนตามแทรก/ลบ |
| `gridGeometry.test.ts` | 12 | พิกัดพิกเซลของแถว-คอลัมน์ (รวมความกว้าง/สูงจากไฟล์ที่นำเข้า), แถวที่ถูกกรองซ่อนไม่กินความสูง, จุดวางกราฟใหม่และการเยื้องไม่ให้ทับกัน |
...
- [x] **กราฟ/แผนภูมิ** — ทำแล้ว (ดูหัวข้อ ✨ ฟีเจอร์): แท่ง เส้น วงกลม วาดจากช่วงและอัปเดตตามค่าจริง
      วางบนตาราง ลากย้ายและย่อขยายได้ ยังเหลือ: ส่งกราฟออกไปกับ `.xlsx`/PDF และผูกตำแหน่งกราฟกับเซลล์
      แทนพิกเซล
